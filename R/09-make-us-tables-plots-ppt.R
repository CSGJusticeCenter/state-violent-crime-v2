...
table_vcr.all <- viol_crime_by_off_us |> 
  ...
  filter(year %in% c(2019,2020)) |>
  group_by(year,crime) |>
  summarise(across(c(cleared_est,pop_total), list(sum))) |> 
  group_by(year) |> 
  mutate(
    rate = cleared_est_1 / pop_total_1 * 1e4 #want the rate at 10K
  ) 
...
table_vcr.gte90_21_ct <- viol_crime_2021_by_off_state |> 
  left_join(offense_pal, by = "crime") |>
  filter(state_abb %in% states.gte90) |>
  select(state_abb, crime, cleared_est) |>
  rename(cleared_est21 = cleared_est)

table_vcr.gte90_20_ct <- viol_crime_by_off_state |> 
  left_join(offense_pal, by = "crime") |> 
  filter(year == 2020 & state_abb %in% states.gte90) |>
  select(state_abb, crime, cleared_est) |>
  rename(cleared_est20 = cleared_est)

table_ct <- merge(table_vcr.gte90_20_ct,table_vcr.gte90_21_ct, by = c("state_abb", "crime")) |>
  mutate(
    decrease = ifelse(cleared_est20-cleared_est21>0,1,0),
    increase = ifelse(cleared_est20-cleared_est21<0,1,0)
  ) |>
  group_by(crime) |>
  summarise(across(c(decrease,increase),list(sum)))
